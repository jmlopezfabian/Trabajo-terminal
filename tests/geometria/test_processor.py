"""Integration tests for vnp46a1 processor with mocked download and I/O."""
...
from vnp46a1.geometria.processor import SatelliteProcessor
...
class TestSatelliteProcessor:
    def test_get_measures_returns_dict_when_mocks_ok(self, sample_hdf5_path):
        ...
        with patch("vnp46a1.geometria.processor.find_file", return_value="http://example.com/file.h5"):
            with patch("vnp46a1.geometria.processor.download_file", return_value=sample_hdf5_path):
                with patch("vnp46a1.geometria.processor.extraer_coordenadas", return_value=coords):
                    with patch("vnp46a1.geometria.processor.recortar_imagen", side_effect=_fake_recortar):
                        with patch("vnp46a1.geometria.processor.completar_bordes", side_effect=_fake_completar_bordes):
                            with patch("vnp46a1.geometria.processor.get_pixeles", side_effect=_fake_get_pixeles):
    ...
    def test_get_measures_returns_none_when_find_file_fails(self):
        with patch("vnp46a1.geometria.processor.find_file", return_value=None):
            proc = SatelliteProcessor("Iztapalapa")
            result = proc.get_measures("01-01-24", "h08v07", show_plots=False)
    ...
    def test_get_measures_returns_none_when_download_fails(self):
        with patch("vnp46a1.geometria.processor.find_file", return_value="http://example.com/file.h5"):
            with patch("vnp46a1.geometria.processor.download_file", return_value=None):
                proc = SatelliteProcessor("Iztapalapa")
                result = proc.get_measures("01-01-24", "h08v07", show_plots=False)
        assert result is None
    ...
    def test_get_measures_returns_none_when_extraer_coordenadas_fails(self, sample_hdf5_path):
        with patch("vnp46a1.geometria.processor.find_file", return_value="http://example.com/file.h5"):
            with patch("vnp46a1.geometria.processor.download_file", return_value=sample_hdf5_path):
                with patch("vnp46a1.geometria.processor.extraer_coordenadas", return_value=None):
                    proc = SatelliteProcessor("Iztapalapa")
                    result = proc.get_measures("01-01-24", "h08v07", show_plots=False)
        assert result is None
            ...
            with patch("vnp46a1.geometria.processor.download_file", return_value=sample_hdf5_path):
                with patch("vnp46a1.geometria.processor.extraer_coordenadas", return_value=coords):
                    with patch("vnp46a1.geometria.processor.recortar_imagen", side_effect=_fake_recortar):
                        with patch("vnp46a1.geometria.processor.completar_bordes", side_effect=_fake_completar_bordes):
                            with patch("vnp46a1.geometria.processor.get_pixeles", side_effect=_fake_get_pixeles):
                                with patch("vnp46a1.geometria.processor.os.remove"):
                                    proc = SatelliteProcessor("Iztapalapa")
